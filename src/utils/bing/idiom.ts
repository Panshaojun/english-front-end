...
export const getIdioms=(dom: HTMLDocument)=>{
    ...
    if(idiomDom){
        ...
        for(let i=0,len=childs.length/2;i<len;i++){
            const keyDom=childs[i*2].querySelector('.defitemtitle');
            const valDom=childs[i*2+1].querySelector('.defitemcon');
            const examItem=childs[i*2+1].querySelectorAll('.examitem');
